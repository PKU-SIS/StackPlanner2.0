...
### 3. Chart Generation
Write the payload to a JSON file under `/mnt/user-data/workspace/`, then invoke
the bundled script using its fixed mounted path. Save the result as a real file
under `/mnt/user-data/outputs/` so it remains previewable and downloadable after
...
node /mnt/skills/public/chart-visualization/scripts/generate.js \
  /mnt/user-data/workspace/chart-spec.json \
  --output-file /mnt/user-data/outputs/chart.png
```

Do not install matplotlib, Python chart packages, npm packages, or system fonts.
This Skill is self-contained. If the bundled generator fails, report that error
once instead of switching plotting stacks or repeatedly installing dependencies.

### 4. Result Return
The script prints the saved output path and the source URL. Verify that the
output file exists, then return:

- The `/mnt/user-data/outputs/...` image path as the primary artifact.
- The source image URL.
- The complete `args` (specification) used for generation.

For several charts, run the script once per chart with distinct descriptive
output names. Do not overwrite one chart with another.
