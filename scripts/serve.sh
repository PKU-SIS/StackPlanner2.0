...
# Local model/Gateway calls must never be sent through an operator's global
# HTTP proxy. This is especially important for vLLM on 127.0.0.1: a proxied
# request can appear as a model timeout even though the service is healthy.
_no_proxy_value="${NO_PROXY:-${no_proxy:-}}"
for _no_proxy_host in 127.0.0.1 localhost; do
    case ",$_no_proxy_value," in
        *",$_no_proxy_host,"*) ;;
        *) _no_proxy_value="${_no_proxy_value:+$_no_proxy_value,}$_no_proxy_host" ;;
    esac
done
export NO_PROXY="$_no_proxy_value"
export no_proxy="$_no_proxy_value"
...
_is_port_listening() {
    ...
    if command -v lsof >/dev/null 2>&1; then
        # Some lsof builds return status 0 with an empty result set. Requiring
        # an actual PID avoids falsely reporting every free port as occupied.
