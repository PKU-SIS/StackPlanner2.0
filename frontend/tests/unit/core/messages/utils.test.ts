...
test("treats StackPlanner delegation as a visible subagent group", () => {
  const message = {
    id: "ai-delegate",
    type: "ai",
    content: "",
    tool_calls: [
      {
        id: "tool-delegate",
        name: "sp_delegate",
        args: { target_agent: "researcher", task: "Find official sources" },
      },
    ],
  } as Message;

  expect(hasSubagent(message as Extract<Message, { type: "ai" }>)).toBe(true);
  expect(getMessageGroups([message]).map((group) => group.type)).toEqual([
    "assistant:subagent",
  ]);
});
