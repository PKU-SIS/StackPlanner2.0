...
describe("stepsForDisplay", () => {
  ...
  it("keeps blank AI steps when they reveal the requested tool and its args", () => {
    const steps = [
      messageToStep(
        {
          type: "ai",
...
describe("latestActiveSubtaskToolCall", () => {
  it("returns the newest requested search while the AI tool-call step is latest", () => {
    const steps = [
      messageToStep({ type: "tool", name: "read_file", content: "done" }, 1),
      messageToStep(
        {
          type: "ai",
          content: "",
    ...
    expect(latestActiveSubtaskToolCall(steps)).toEqual({
      name: "web_search",
      args: { query: "Qwen tool calling" },
    });
  });

  it("clears the active search after its tool result arrives", () => {
    const steps = [
      messageToStep(
        {
          type: "ai",
          content: "",
          tool_calls: [{ name: "web_search", args: { query: "gold price" } }],
        },
        1,
      ),
      messageToStep(
    ...
    expect(latestActiveSubtaskToolCall(steps)).toBeUndefined();
  });
});
