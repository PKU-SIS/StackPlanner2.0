...
describe("parseSubtaskResult", () => {
  ...
  it("marks historical StackPlanner delegate acknowledgements terminal", () => {
    expect(
      parseSubtaskResult(
        JSON.stringify({
    ...
    ).toEqual({ status: "failed", error: "search failed" });
  });
});
...
describe("parseSubtaskResult — structured additional_kwargs (preferred path)", () => {
  ...
  it("returns downloadable output artifacts and rejects non-output paths", () => {
