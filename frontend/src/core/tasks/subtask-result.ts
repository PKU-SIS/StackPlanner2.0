...
export const SP_ARTIFACT_PATHS_KEY = "sp_artifact_paths";
/**
...
export function parseSubtaskResult(
  ...
  const artifacts = readArtifactPaths(additionalKwargs);
  const structured = readStructuredStatus(additionalKwargs);
  if (!structured) {
    if (!hasStructuredSubagentMetadata(additionalKwargs)) {
      return withArtifacts(parseLegacyTaskResult(text.trim()), artifacts);
    }
    return withArtifacts({ status: "in_progress" }, artifacts);
  }
  ...
  return withArtifacts(update, artifacts);
}

function readArtifactPaths(
  additionalKwargs: Record<string, unknown> | null | undefined,
): string[] | undefined {
  const value = additionalKwargs?.[SP_ARTIFACT_PATHS_KEY];
...
function withArtifacts(
  update: SubtaskResultUpdate,
  artifacts: string[] | undefined,
): SubtaskResultUpdate {
  return artifacts ? { ...update, artifacts } : update;
}

function parseLegacyTaskResult(trimmed: string): SubtaskResultUpdate {
  // StackPlanner's original sp_delegate adapter persisted only this action
  // acknowledgement after the synchronous subagent execution had returned.
  // Treat it as terminal so historical/reloaded cards do not spin forever.
  // New runs carry the structured subagent_status metadata handled above.
  try {
    const actionResult = JSON.parse(trimmed) as {
      action_type?: unknown;
      next_step?: unknown;
      error?: unknown;
    };
    if (actionResult.action_type === "DELEGATE") {
      if (typeof actionResult.error === "string" && actionResult.error.trim()) {
        return { status: "failed", error: actionResult.error.trim() };
      }
      if (
        actionResult.error == null &&
        (actionResult.next_step === "continue" ||
          actionResult.next_step === "finish")
      ) {
        return { status: "completed" };
      }
    }
  } catch {
    // Legacy task results are normally plain text; continue with prefix parsing.
  }
