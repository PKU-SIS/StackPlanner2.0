 ...
 * - AI steps are kept when they carry visible reasoning text or tool-call
 *   requests. Keeping a blank tool-call turn preserves safe display arguments
 *   such as the current web-search query before and after the result arrives;
...
/**
 * Return the newest tool request only while its assistant step is the latest
 * streamed subagent step. Once a ToolMessage arrives the call has completed,
 * so the collapsed card can return to the generic running state instead of
 * claiming that an old search is still active.
 */
export function latestActiveSubtaskToolCall(
  steps: SubtaskStep[] | undefined,
): SubtaskStepToolCall | undefined {
  if (!steps?.length) {
    return undefined;
  }
  const latest = steps.reduce((current, step) =>
    step.message_index > current.message_index ? step : current,
  );
  if (latest.kind !== "ai" || !latest.tool_calls?.length) {
    return undefined;
  }
  return latest.tool_calls[latest.tool_calls.length - 1];
}
