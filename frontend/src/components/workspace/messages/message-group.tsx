...
import { getSPActionType, isSPActionToolName } from "@/core/tools/sp-actions";
...
function ToolCall({
  ...
  } else if (name === "ask_clarification" || name === "sp_ask_human") {
    return (
      <ChainOfThoughtStep
        key={id}
  ...
  } else if (isSPActionToolName(name)) {
    return (
      <ChainOfThoughtStep
        key={id}
        label={resolveLabel(t.toolCalls.spAction(getSPActionType(name)))}
        icon={WaypointsIcon}
      ></ChainOfThoughtStep>
    );
  } else {
