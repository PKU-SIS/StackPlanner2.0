...
_BRANCH_CONVERTIBLE_UPLOAD_EXTENSIONS = {".pdf", ".ppt", ".pptx", ".xls", ".xlsx", ".doc", ".docx"}
...
def _checkpoint_file_timestamp(checkpoint: dict[str, Any]) -> float | None:
...
def _normalize_branch_virtual_path(value: Any) -> str | None:
...
def _checkpoint_referenced_file_paths(checkpoint: dict[str, Any]) -> list[str]:
    """Return trusted file references materialized by a checkpoint.

    Historical branches cannot clone the source's entire current workspace: it
    may contain files created by later turns. The checkpoint does, however,
    persist generated artifact paths and structured upload metadata. Those
    references are safe to carry onto the branch and are required for inherited
    Markdown images, downloads, and uploaded-document follow-up questions.
    """
    channel_values = checkpoint.get("channel_values", {}) or {}
    references: list[str] = []
    ...
    if isinstance(artifacts, list):
        for value in artifacts:
            if path := _normalize_branch_virtual_path(value):
        ...
        for message in messages:
            ...
            for file_info in files:
                ...
                if not isinstance(filename, str) or not filename or Path(filename).name != filename:
                    continue
                references.append(f"/mnt/user-data/uploads/{filename}")
                if Path(filename).suffix.lower() in _BRANCH_CONVERTIBLE_UPLOAD_EXTENSIONS:
                    references.append(f"/mnt/user-data/uploads/{Path(filename).with_suffix('.md').name}")

    return list(dict.fromkeys(references))


def _copy_checkpoint_files_sync(
    paths: Paths,
    ...
    *,
    ...
    references = _checkpoint_referenced_file_paths(checkpoint)
    if not references:
        return "skipped_historical_turn"

    checkpoint_timestamp = _checkpoint_file_timestamp(checkpoint)
    if checkpoint_timestamp is None:
        logger.warning("Historical branch checkpoint has no valid timestamp; skipping referenced files")
        return "checkpoint_files_unavailable"

    copied = 0
    paths.ensure_thread_dirs(target_thread_id, user_id=user_id)
    for virtual_path in references:
        try:
            source = paths.resolve_virtual_path(source_thread_id, virtual_path, user_id=user_id)
            if not source.exists():
                legacy_source = paths.resolve_virtual_path(source_thread_id, virtual_path)
                if legacy_source.exists():
                    source = legacy_source
            target = paths.resolve_virtual_path(target_thread_id, virtual_path, user_id=user_id)
        except ValueError:
            logger.warning("Skipping invalid checkpoint file reference %s", virtual_path)
            continue

        try:
            if not source.is_file() or source.is_symlink():
                continue
            # A path may have been overwritten by a later turn. Fail closed
            # instead of copying future content into a historical branch.
            if source.stat().st_mtime > checkpoint_timestamp:
                logger.warning("Skipping checkpoint file modified after the branch point: %s", virtual_path)
                continue
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source, target)
            copied += 1
        except OSError:
            logger.warning("Failed to copy checkpoint file %s", virtual_path, exc_info=True)

    return "checkpoint_files_best_effort" if copied else "checkpoint_files_unavailable"
...
async def _copy_checkpoint_files(
    ...
    try:
        ...
        logger.warning(
            "Failed to copy checkpoint files for branch %s -> %s",
            sanitize_log_param(source_thread_id),
            sanitize_log_param(target_thread_id),
            exc_info=True,
        )
...
async def branch_thread(thread_id: str, body: ThreadBranchRequest, request: Request) -> ThreadBranchResponse:
    ...
    if branch_from_latest_turn:
        ...
        workspace_clone_mode = await _copy_checkpoint_files(thread_id, new_thread_id, checkpoint)
