...
class DbRunEventStore(RunEventStore):
    def __init__(self, session_factory: async_sessionmaker[AsyncSession], *, max_trace_content: int = 10240):
        ...
        # SQLite cannot row-lock ``SELECT max(seq)``. Journal batches and
        # persisted subagent-step batches can therefore read the same max and
        # allocate duplicate sequence numbers. Locks are scoped by event loop
        # so a long-lived store remains safe in tests/workers that create a new
        # loop, and by thread id so unrelated conversations still write in
        # parallel. PostgreSQL keeps its advisory DB lock as the cross-process
        # authority; this local lock also gives deterministic in-process order.
        self._write_locks_guard = threading.Lock()
        self._write_locks_by_loop: weakref.WeakKeyDictionary[
            asyncio.AbstractEventLoop,
            dict[str, asyncio.Lock],
        ] = weakref.WeakKeyDictionary()

    @asynccontextmanager
    async def _thread_write_lock(self, thread_id: str) -> AsyncIterator[None]:
        loop = asyncio.get_running_loop()
        with self._write_locks_guard:
            locks = self._write_locks_by_loop.get(loop)
            if locks is None:
                locks = {}
                self._write_locks_by_loop[loop] = locks
    ...
    async def put(self, *, thread_id, run_id, event_type, category, content="", metadata=None, created_at=None):  # noqa: D401
        ...
        async with self._thread_write_lock(thread_id):
