...
class RunJournal(BaseCallbackHandler):
    ...
    def __init__(
        ...
        *,
        ...
        # Subagents may report usage and SP events from worker threads while
        # the lead graph continues on the ASGI loop. Protect compound buffer
        # and accumulator operations so parallel delegation cannot lose events
        # or token increments during a concurrent flush.
    ...
    def _schedule_progress_flush(self) -> None:
        ...
        if self._owner_loop is not None and current_loop is not self._owner_loop:
            # Parallel SP delegates report usage from the persistent subagent
            # loop. RunManager and its SQL sessions belong to the worker loop,
            # so marshal progress persistence back to that owner instead of
            # creating a task on the foreign loop.
            if not self._owner_loop.is_closed():
                self._owner_loop.call_soon_threadsafe(self._schedule_progress_flush)
            return
        now = time.monotonic()
        ...
        if current_loop is None:
            return
        self._progress_dirty = False
        self._pending_progress_task = current_loop.create_task(self._flush_progress_async(snapshot=self.get_completion_data()))
