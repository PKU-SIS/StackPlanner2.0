...
_LONG_LIVED_SERVER_PATTERN = re.compile(
    r"(?:"
    r"\b(?:server|service|daemon)\b"
    r"|\b(?:uvicorn|gunicorn|hypercorn|daphne)\b"
    r"|\bpython(?:3)?\s+-m\s+http\.server\b"
    r"|\b(?:npm|pnpm|yarn|bun)\b[^\n;&|]{0,40}\b(?:dev|serve|start)\b"
    r"|\b(?:next|vite|webpack|parcel|streamlit|gradio)\b[^\n;&|]{0,24}\b(?:dev|serve|run)?\b"
    r"|长期运行|长时间运行|后台服务|启动服务|开发服务器|网站服务|监听端口"
    r")",
    re.IGNORECASE,
)
_BACKGROUND_EXECUTION_REJECTION = (
    "Background execution is reserved for an explicit long-lived server. "
    "Run calculations, scripts, tests, and verification in the foreground so "
    "the timeout, exit status, and output remain observable. If computation "
    "times out, fix or bound the algorithm instead of appending '&'."
)


def _has_unquoted_background_operator(command: str) -> bool:
    """Return whether *command* contains a shell background ``&`` operator.

    This deliberately distinguishes a real, unquoted operator from ``&&``,
    file-descriptor redirects such as ``2>&1``/``&>``, escaped ampersands, and
    ampersands inside quotes. An unquoted ampersand in a URL still counts:
    POSIX shells also interpret it as an operator unless the URL is quoted.
    """
    ...
    for index, char in enumerate(command):
        ...
        if char == '"' and not single_quoted:
            ...
            continue
        if char != "&" or single_quoted or double_quoted:
            continue
...
def _background_execution_error(description: str, command: str) -> str | None:
    """Reject backgrounded finite work while retaining server startup support."""

    if not _has_unquoted_background_operator(command):
        return None
    if _LONG_LIVED_SERVER_PATTERN.search(f"{description}\n{command}"):
        return None
    return _BACKGROUND_EXECUTION_REJECTION
...
def bash_tool(runtime: Runtime, description: str, command: str) -> str:
    ...
    try:
        ...
        background_error = _background_execution_error(description, command)
        if background_error:
            return f"Error: {background_error}"
