...
def build_middlewares(
    ...
    *,
    ...
    Args:
        ...
        control_plane_only: Keep generic conversation compaction, date, safety,
            and accounting middleware, but do not inject full Skill bodies,
            duplicate delegation/Skill ledgers, or queue automatic long-term
            memory writes. Intended for orchestration agents whose own control
            state and explicit recall path are authoritative.
    ...
    middlewares.append(
        DynamicContextMiddleware(
            ...
            inject_memory=inject_long_term_memory,
        )
    )
    ...
    middlewares.append(
        DurableContextMiddleware(
            ...
            include_delegations=not control_plane_only,
            include_skills=not control_plane_only,
        )
    )
...
def _make_lead_agent(
    ...
    *,
    ...
    is_plan_mode = cfg.get("is_plan_mode", False)
    subagent_enabled = cfg.get("subagent_enabled", False)
    if not include_base_tools:
        # This is a model-schema security boundary, not a request preference.
        # Runtime context has higher merge precedence than ``configurable``, so
        # enforce it here as well to prevent middleware from reintroducing the
        # native ``task`` or ``write_todos`` tool.
        is_plan_mode = False
        subagent_enabled = False
        middleware_config = dict(config)
        middleware_config["configurable"] = {
            **dict(config.get("configurable", {}) or {}),
            "is_plan_mode": False,
            "subagent_enabled": False,
        }
        middleware_config["context"] = {
    ...
    if is_bootstrap:
        ...
        raw_tools = get_available_tools(model_name=model_name, subagent_enabled=subagent_enabled, app_config=resolved_app_config) + [setup_agent] if include_base_tools else []
        ...
        if include_base_tools and skill_setup.describe_skill_tool:
        ...
        return create_agent(
            ...
            middleware=build_middlewares(
                ...
                inject_long_term_memory=inject_long_term_memory,
    ...
    agent_extra_tools = [update_agent] if include_base_tools and agent_name and not is_webhook_channel else []
    # StackPlanner can reuse this factory with only its explicit control-action
    # vocabulary.  The default lead-agent path keeps the existing base tools.
    raw_tools = (
        get_available_tools(
            model_name=model_name,
    ...
    return create_agent(
        ...
        system_prompt="\n\n".join(section for section in (prompt_prefix, base_prompt) if section),
        state_schema=ThreadState,
    )
