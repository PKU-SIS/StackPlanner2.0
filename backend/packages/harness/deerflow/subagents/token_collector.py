...
class SubagentTokenCollector(BaseCallbackHandler):
    ...
    def __init__(
        ...
        caller: str,
        *,
        record_observer: Callable[[dict[str, int | str | None]], None] | None = None,
    ):
        super().__init__()
        self.caller = caller
        ...
        for generation in response.generations:
            for gen in generation:
                ...
                if self._record_observer is not None:
                    try:
                        self._record_observer(dict(record))
                    except Exception:
                        # Usage telemetry must never fail delegated work.
                        logger.debug("Subagent token record observer failed", exc_info=True)
                return
