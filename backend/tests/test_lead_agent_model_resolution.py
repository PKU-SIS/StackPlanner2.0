...
def test_internal_make_lead_agent_can_bind_only_explicit_control_tools(monkeypatch):
    from deerflow.sp.agent_tools import build_sp_control_tools

    app_config = _make_app_config([_make_model("control-model", supports_thinking=False)])
    import deerflow.tools as tools_module

    def _reject_base_tools(**kwargs):
        raise AssertionError(f"base DeerFlow tools must not be loaded: {kwargs}")

    monkeypatch.setattr(tools_module, "get_available_tools", _reject_base_tools)
    monkeypatch.setattr(lead_agent_module, "_load_enabled_skills_for_tool_policy", lambda *args, **kwargs: [])
    middleware_configs = []

    def _capture_middlewares(config, *args, **kwargs):
        middleware_configs.append(config)
        return []
    ...
    result = lead_agent_module._make_lead_agent(
        {
            "configurable": {"model_name": "control-model"},
            "context": {"is_plan_mode": True, "subagent_enabled": True},
        },
        app_config=app_config,
        extra_tools=build_sp_control_tools(),
        prompt_prefix="SP-only prompt",
    ...
    assert result["system_prompt"] == "SP-only prompt"
    assert middleware_configs[0]["context"]["is_plan_mode"] is False
    assert middleware_configs[0]["context"]["subagent_enabled"] is False
...
def test_control_plane_middlewares_keep_conversation_summary_without_skill_or_memory_side_channels(monkeypatch):
    app_config = _make_app_config([_make_model("safe-model", supports_thinking=False)])
    ...
    assert not any(isinstance(middleware, SkillActivationMiddleware) for middleware in middlewares)
    ...
    assert durable._include_summary is True
    assert durable._include_delegations is False
    assert durable._include_skills is False
