...
def test_memory_lookup_is_skipped_when_injection_is_disabled():
    mw = _make_middleware(inject_memory=False)
    state = {"messages": [HumanMessage(content="Hi", id="msg-1")]}

    with (
        mock.patch(
            "deerflow.agents.lead_agent.prompt._get_memory_context",
            side_effect=AssertionError("long-term memory must stay on demand"),
        ),
        mock.patch("deerflow.agents.middlewares.dynamic_context_middleware.datetime") as mock_dt,
    ):
        mock_dt.now.return_value.strftime.return_value = "2026-05-08, Friday"
