...
class TestSubagentTokenCollector:
    ...
    def test_notifies_observer_as_each_usage_record_arrives(self):
        observed = []
        collector = SubagentTokenCollector(
            caller="subagent:test",
            record_observer=observed.append,
        )
        usage = {"input_tokens": 100, "output_tokens": 50, "total_tokens": 150}
        ...
        assert observed[0]["total_tokens"] == 150
        assert observed[0] is not collector.snapshot_records()[0]

    def test_observer_failure_does_not_break_collection(self):
        collector = SubagentTokenCollector(
            caller="subagent:test",
            record_observer=lambda _record: (_ for _ in ()).throw(RuntimeError("telemetry down")),
        )
        usage = {"input_tokens": 10, "output_tokens": 5, "total_tokens": 15}

        collector.on_llm_end(_make_llm_response("Hi", usage=usage), run_id=uuid4())

        assert collector.snapshot_records()[0]["total_tokens"] == 15
