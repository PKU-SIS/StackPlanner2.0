...
class TestDurableContextInjection:
    ...
    def test_fresh_user_turn_does_not_reinject_abandoned_conversation_summary(self):
        model = RecordingFakeModel(responses=[AIMessage(content="ok")])
        agent = create_agent(
            model=model,
            tools=[],
            middleware=[DurableContextMiddleware(include_delegations=False, include_skills=False)],
            state_schema=ThreadState,
        )
        ...
        agent.invoke(
            {
                ...
                "summary_text": "ABANDONED_RUN_SUMMARY",
            },
            context={"fresh_user_turn_after_terminal": True},
        )

        rendered = "\n".join(str(message.content) for message in model.received[-1])
        assert "ABANDONED_RUN_SUMMARY" not in rendered
