...
@pytest.mark.parametrize(
    ...
    [
        (
            "run the calculation",
            "python /mnt/user-data/workspace/calculate.py > /mnt/user-data/workspace/calculate.log 2>&1 &",
        ),
        ("run tests", "pytest /mnt/user-data/workspace/tests &"),
        ("verify the generated result", "node /mnt/user-data/workspace/verify.js&"),
    ],
)
def test_background_execution_guard_rejects_finite_work(description: str, command: str) -> None:
    error = _background_execution_error(description, command)

    assert error is not None
    assert "Run calculations, scripts, tests, and verification in the foreground" in error
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "uvicorn app:app --port 8080 > /mnt/user-data/workspace/server.log 2>&1 &",
        ),
        (
            "启动长期运行的网站服务",
            "python /mnt/user-data/workspace/app.py > /mnt/user-data/workspace/server.log 2>&1 &",
        ),
    ],
)
def test_background_execution_guard_allows_explicit_long_lived_servers(description: str, command: str) -> None:
    assert _background_execution_error(description, command) is None
...
@pytest.mark.parametrize(
    ...
    [
        "echo a && echo b",
        "python /mnt/user-data/workspace/calculate.py > /mnt/user-data/workspace/result.log 2>&1",
        "echo '&'",
        'echo "&"',
        r"echo \&",
        'curl "https://example.com/search?a=1&b=2"',
        "echo value &> /mnt/user-data/workspace/result.log",
    ],
)
def test_background_execution_guard_ignores_non_background_ampersands(command: str) -> None:
    assert _background_execution_error("run a foreground command", command) is None


def test_bash_tool_rejects_backgrounded_calculation_before_execution(monkeypatch) -> None:
    runtime = SimpleNamespace(
        state={"sandbox": {"sandbox_id": "local"}, "thread_data": _THREAD_DATA.copy()},
        context={"thread_id": "thread-1"},
    )
    ...
    sandbox = SimpleNamespace(
        execute_command=lambda *args, **kwargs: execute_command("backgrounded calculation should not execute"),
    )
    ...
    result = bash_tool.func(
        ...
        command="python /mnt/user-data/workspace/calculate.py > /mnt/user-data/workspace/result.log 2>&1 &",
    )

    assert result.startswith("Error: Background execution is reserved")
