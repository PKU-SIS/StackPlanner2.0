...
class TestSubagentLimitMiddlewareInit:
    ...
    def test_custom_delegation_tool_names(self):
        mw = SubagentLimitMiddleware(tool_names={"sp_delegate"})

        assert mw.tool_names == frozenset({"sp_delegate"})
...
class TestTruncateTaskCalls:
    ...
    def test_stackplanner_delegate_calls_use_the_same_concurrency_guard(self):
        mw = SubagentLimitMiddleware(max_concurrent=2, tool_names={"sp_delegate"})
        msg = AIMessage(
            content="",
            tool_calls=[
                _other_call("sp_delegate", "sp1"),
                _other_call("sp_delegate", "sp2"),
                _other_call("sp_delegate", "sp3"),
                _task_call("native-task"),
            ],
        )
        ...
        assert result is not None
        updated = result["messages"][0].tool_calls
        assert [call["id"] for call in updated if call["name"] == "sp_delegate"] == [
            "sp1",
            "sp2",
        ]
        assert any(call["id"] == "native-task" for call in updated)
