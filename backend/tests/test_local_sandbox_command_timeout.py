...
def test_sandbox_config_exposes_command_timeout_default():
    ...
    assert cfg.bash_command_timeout == 120
...
def test_bash_tool_description_guides_backgrounding_long_lived_processes():
    ...
    assert "calculations, scripts, tests, and verification in the foreground" in description
    assert "never append `&`" in description
