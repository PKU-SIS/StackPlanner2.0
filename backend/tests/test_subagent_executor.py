...
class TestAsyncExecutionPath:
    ...
    @pytest.mark.anyio
    async def test_aexecute_reports_each_captured_step_once(self, classes, base_config, mock_agent, msg):
        SubagentExecutor = classes["SubagentExecutor"]
        observed = []
        first = msg.ai("First response", "msg-1")
        second = msg.tool("Search result", "call-1", name="web_search", msg_id="msg-2")
        chunks = [
            {"messages": [msg.human("Task"), first]},
            {"messages": [msg.human("Task"), first, second]},
        ]
        ...
        executor = SubagentExecutor(
            ...
            tools=[],
            thread_id="test-thread",
            step_observer=lambda message, index: observed.append((message["id"], index)),
        )

        with patch.object(executor, "_create_agent", return_value=mock_agent):
            await executor._aexecute("Task")

        assert observed == [("msg-1", 1), ("msg-2", 2)]

    def test_platform_secret_binding_requires_public_declaring_skill(self, classes, base_config):
        SubagentExecutor = classes["SubagentExecutor"]
        public_dir = Path("/tmp/video-generation")
        public_skill = Skill(
            name="video-generation",
            description="Generate videos",
            license=None,
            skill_dir=public_dir,
            skill_file=public_dir / "SKILL.md",
            relative_path=Path("video-generation"),
            category=SkillCategory.PUBLIC,
            enabled=True,
            required_secrets=(SecretRequirement("MINIMAX_API_KEY"),),
        )
        shadow = Skill(
            name="video-generation",
        ...
        undeclared = Skill(
            name="other-public",
            description="No credential declaration",
            license=None,
            skill_dir=public_dir,
            skill_file=public_dir / "SKILL.md",
            relative_path=Path("other-public"),
            category=SkillCategory.PUBLIC,
            enabled=True,
        )
        executor = SubagentExecutor(
            config=base_config,
            tools=[],
            platform_skill_secrets={"MINIMAX_API_KEY": "server-secret"},
        )

        assert executor._bind_platform_skill_secrets([public_skill]) == {"MINIMAX_API_KEY": "server-secret"}
        assert executor._bind_platform_skill_secrets([shadow, undeclared]) == {}
...
class TestSyncExecutionPath:
    ...
    def test_force_isolated_loop_applies_outside_running_loop(self, classes, base_config):
        """SP worker threads still use one stable loop for shared async tools."""
        SubagentExecutor = classes["SubagentExecutor"]
        expected = object()
        executor = SubagentExecutor(
            config=base_config,
        ...
        with patch.object(executor, "_execute_in_isolated_loop", return_value=expected) as isolated:
            result = executor.execute("Task")

        assert result is expected
        isolated.assert_called_once_with("Task", None)
...
class TestCooperativeCancellation:
    ...
    def test_isolated_execution_stops_when_parent_run_aborts(self, executor_module, classes, base_config):
        """A cancelled parent must stop an isolated synchronous child promptly."""
        ...
        executor = SubagentExecutor(
            ...
            tools=[],
            thread_id="test-thread",
            parent_abort_event=abort_event,
            force_isolated_loop=True,
        )
        ...
        assert result.status.value == SubagentStatus.CANCELLED.value
        assert result.error == "Cancelled because the parent run stopped"
        assert result.cancel_event.is_set()
