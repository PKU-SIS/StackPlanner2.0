...
from deerflow.agents.thread_state import (
    ...
    merge_sandbox,
    merge_skill_context,
    merge_sp_artifact_refs,
    merge_sp_handler_result,
...
class TestMergeSPTaskMemory:
    ...
    def test_parallel_action_snapshots_merge_by_entry_id(self):
        existing = {"version": 1, "entries": [{"id": "base", "action": "think"}]}
        first = {
            "version": 1,
            "entries": [*existing["entries"], {"id": "observe-a", "action": "observe"}],
            SP_CONCURRENT_MERGE_MODE_KEY: True,
        }
        second = {
            "version": 1,
            "entries": [*existing["entries"], {"id": "observe-b", "action": "observe"}],
            SP_CONCURRENT_MERGE_MODE_KEY: True,
        }

        merged = merge_sp_task_memory(merge_sp_task_memory(existing, first), second)

        assert [entry["id"] for entry in merged["entries"]] == ["base", "observe-a", "observe-b"]
        assert SP_CONCURRENT_MERGE_MODE_KEY not in merged

    def test_summary_tombstone_prevents_parallel_stale_snapshot_from_readding_sources(self):
        existing = {
            "version": 1,
            "entries": [
                {"id": "old", "action": "observe", "status": "active"},
                {"id": "recent", "action": "think", "status": "active"},
            ],
        }
        summarized = {
            "version": 1,
        ...
        sibling = {
            ...
            "entries": [
                *existing["entries"],
                {"id": "sibling", "action": "observe", "status": "active"},
            ],
            SP_CONCURRENT_MERGE_MODE_KEY: True,
        }

        merged = merge_sp_task_memory(merge_sp_task_memory(existing, summarized), sibling)

        assert {entry["id"] for entry in merged["entries"]} == {"recent", "summary", "sibling"}


class TestMergeSPArtifactRefs:
    def test_parallel_artifacts_are_all_retained_in_history(self):
        first = {
            "report": {
                "artifact_id": "artifact-a",
                "type": "report",
                "version": 1,
                "virtual_path": "/mnt/user-data/outputs/a.md",
            },
            "_history": [],
            SP_CONCURRENT_MERGE_MODE_KEY: True,
        }
        second = {
            "report": {
                ...
                "type": "report",
                "version": 1,
                "virtual_path": "/mnt/user-data/outputs/b.md",
            },
            "_history": [],
            SP_CONCURRENT_MERGE_MODE_KEY: True,
        }

        merged = merge_sp_artifact_refs(merge_sp_artifact_refs(None, first), second)

        assert {item["artifact_id"] for item in merged["_history"]} == {"artifact-a", "artifact-b"}
        assert sum(bool(item["is_current"]) for item in merged["_history"]) == 1

    def test_unmarked_update_remains_exact_replacement_for_backtrack(self):
        existing = {"report": {"artifact_id": "new"}}
        restored = {"report": {"artifact_id": "old"}}

        assert merge_sp_artifact_refs(existing, restored) == restored


def test_sp_handler_results_form_one_batch_for_same_iteration():
    first = {
        "action_id": "a",
    ...
    merged = merge_sp_handler_result(first, second)

    assert merged["action_type"] == "BATCH"
    assert merged["parallel_action_count"] == 2
    assert merged["next_step"] == "error_recoverable"


def test_parallel_finish_cannot_skip_unfinished_sibling_action():
    finish = {
        "action_id": "finish",
        "action_type": "FINISH",
        "next_step": "finish",
    ...
    merged = merge_sp_handler_result(finish, delegate)

    assert merged["action_type"] == "BATCH"
    assert merged["next_step"] == "continue"


def test_thread_state_accepts_parallel_sp_action_updates_without_invalid_concurrent_update():
    def branch(action_id):
        return {
            "sp_active_delegate_id": None,
            "sp_task_memory": {
                "version": 1,
                "entries": [{"id": action_id, "action": "observe", "status": "active"}],
                SP_CONCURRENT_MERGE_MODE_KEY: True,
            },
    ...
    builder.add_node("first", lambda state: branch("first"))
    builder.add_node("second", lambda state: branch("second"))
    ...
    assert {entry["id"] for entry in result["sp_task_memory"]["entries"]} == {"first", "second"}
    assert result["sp_last_handler_result"]["parallel_action_count"] == 2
