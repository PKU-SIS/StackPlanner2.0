...
class TestDbRunEventStore:
    ...
    @pytest.mark.anyio
    async def test_concurrent_batches_allocate_unique_thread_sequences(self, tmp_path):
        """Journal and subagent event batches must not race on SQLite max(seq)."""
        from deerflow.persistence.engine import close_engine, get_session_factory, init_engine
        from deerflow.runtime.events.store.db import DbRunEventStore
        ...
        first = [{"thread_id": "t1", "run_id": "r1", "event_type": "journal", "category": "trace"} for _ in range(20)]
        second = [{"thread_id": "t1", "run_id": "r1", "event_type": "subagent", "category": "subagent"} for _ in range(20)]

        await asyncio.gather(store.put_batch(first), store.put_batch(second))
        events = await store.list_events("t1", "r1", limit=100)
